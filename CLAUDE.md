...
which reads every thread's `index.json` and shows each thread's current state plus its
most recent entries, folding the rest away. Both build tools call it at the end of a run,
so whichever thread published last, the front door is right for both. Never hand-edit
`index.html`.

Every Bench page carries the same nav bar -- today / build log / kit / threads -- and day
cards additionally have previous and next. Navigation is chrome, so it is generated by
`bench.py`, never written into a template by hand.
...
`index.html` is small on purpose -- the design lives in the dated cards -- but it is the
page that has to answer "where do I go" in a second, so it carries each thread's current
state and its recent entries rather than one list that grows to hundreds of links.
